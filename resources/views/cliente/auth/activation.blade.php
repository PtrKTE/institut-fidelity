...
@section('title', 'Activation — Prestige by ProNails')
...
<div class="cliente-auth" style="min-height:100vh;padding-bottom:0;">
    <div class="cliente-auth-card">
        ...
        <h5 class="auth-title">Activez votre compte</h5>
        <p class="text-center text-muted small mb-3">Un code OTP a ete envoye a <strong>{{ $email }}</strong></p>

        @if($errors->any())
            <div class="alert alert-danger small py-2">{{ $errors->first() }}</div>
        @endif

        <form method="POST" action="{{ route('espace-cliente.activation') }}">
            @csrf
            <input type="hidden" name="email" value="{{ $email }}">
            <div class="mb-3">
                <label class="form-label small">Code OTP (6 chiffres)</label>
            ...
            <button type="submit" class="btn-fid-primary w-100 py-2">Activer mon compte</button>
        </form>

        <div class="text-center mt-3">
            <button class="btn btn-link btn-sm text-muted" id="btnResend" disabled>
                Renvoyer le code (<span id="countdown">120</span>s)
            </button>
        </div>
    </div>
</div>
